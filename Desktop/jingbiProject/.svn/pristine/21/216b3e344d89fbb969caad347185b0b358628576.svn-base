...
- (void)configureViewWithMoney:(NSString *)money cny:(NSString *)cny {
    ...
    if (cny.floatValue == 0) {
        self.cnyLB.text = [NSString stringWithFormat:@"≈$%@",@"0.00"];
    }else{
        self.cnyLB.text = [NSString stringWithFormat:@"≈$%@",[WLTools noroundingStringWith:cny.doubleValue afterPointNumber:2]];
    }
