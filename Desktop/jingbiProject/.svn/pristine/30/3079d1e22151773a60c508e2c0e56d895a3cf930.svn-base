...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = SSKJLocalized(@"绑定谷歌验证器", nil);
...
{
    if (nil == _nextButton) {
        _nextButton = [[UIButton alloc]initWithFrame:CGRectMake(ScaleW(12), self.bottomWarningLabel.bottom + ScaleW(88), ScreenWidth - ScaleW(24), ScaleW(45))];
...
//
//    [HttpTool postWithURL:CreateGoogleCommand params:nil success:^(id json) {
//        [MBProgressHUD hideHUDForView:weakSelf.view animated:YES];
//
//        NSString *status = json[@"status"];
//        LSLog(@"json : %@",json);
//        if (status.integerValue == 200) {
//            My_Google_Model *googleModel = [My_Google_Model mj_objectWithKeyValues:json[@"data"]];
...
{
//    [self.qrCodeImageView sd_setImageWithURL:[NSURL URLWithString:googleModel.local_url ]];
