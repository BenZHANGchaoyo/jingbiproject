...
-(UIButton *)securedButton
{
    if (nil == _securedButton) {
        ...
        _securedButton.centerY = self.height - ScaleW(17);
...
-(UITextField *)textField
{
    if (nil == _textField) {
        ...
        _textField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:_textField.placeholder attributes:@{NSForegroundColorAttributeName:kTextDarkBlueColor}];
...
{
    if (nil == _titleLabel) {
        _titleLabel = [WLTools allocLabel:_title font:_font textColor:kTextLightWhiteColor frame:CGRectMake(_leftGap, self.textField.y - ScaleW(19), ScreenWidth - 2 * _leftGap, ScaleW(13.5)) textAlignment:NSTextAlignmentLeft];
