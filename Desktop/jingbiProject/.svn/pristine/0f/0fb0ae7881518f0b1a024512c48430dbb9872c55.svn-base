...
-(UIButton *)introductButton
{
    if (nil == _introductButton) {
        _introductButton = [[UIButton alloc]initWithFrame:CGRectMake(self.dealButton.right, 0, self.width / 2, self.height)];
        [_introductButton setTitle:SSKJLocalized(@"简介", nil) forState:UIControlStateNormal];
        [_introductButton setTitleColor:kTextDarkBlueColor forState:UIControlStateNormal];
...
{
    if (nil == _lineView) {
        _lineView = [[UIView alloc]initWithFrame:CGRectMake(ScaleW(15), self.height - 0.5, self.width - ScaleW(30), 0.5)];
