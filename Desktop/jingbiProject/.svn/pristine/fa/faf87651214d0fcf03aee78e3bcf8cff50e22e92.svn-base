...
{
    if (nil == _unitLabel) {
        _unitLabel = [WLTools allocLabel:SSKJLocalized(@"USDT", nil) font:systemFont(ScaleW(11)) textColor:kTextLightBlueColor frame:CGRectMake(self.width - ScaleW(5) - ScaleW(35), 0, ScaleW(35), self.height) textAlignment:NSTextAlignmentRight];
    }
    return _unitLabel;
}
...
-(UITextField *)numberTextField
{
    if (nil == _numberTextField) {
        ...
        [_numberTextField addTarget:self action:@selector(inputChanged:) forControlEvents:UIControlEventTouchUpInside];
    }
