...
- (LXY_KLine_TipView *)kLine_tipView
{
    if (nil == _kLine_tipView) {
        _kLine_tipView = [[LXY_KLine_TipView alloc]initWithFrame:CGRectMake(self.kLine_MainView.x, self.kLine_MainView.y, 100, 70)];
...
-(UIView *)longPress_HView
{
    if (nil == _longPress_HView) {
        _longPress_HView =[[UIView alloc]initWithFrame:CGRectMake(0, 0, self.kLine_MainView.width, 0.5)];
...
-(void)refreshWithSocketData:(LXY_KLine_DataModel *)socketModel minuteInvital:(NSInteger)minuteInvital
{
    ...
    NSDate *socketDate = [NSDate dateWithTimeIntervalSince1970:socketModel.timestamp.doubleValue / 1000];
    ...
    if (second / 60 >= minuteInvital && minuteInvital != 0) {
        ...
        //        socketModel.time = [self.formatter stringFromDate:[NSDate dateWithTimeIntervalSince1970:socketModel.timestamp.doubleValue]];
        //        NSMutableDictionary *dic = [socketModel mj_keyValues];
        //        [dic setObject:dic[@"price"] forKey:@"closingPrice"];
        ////        [dic setObject:dic[@"open"] forKey:@"close"];
        //        [dic setObject:dic[@"price"] forKey:@"highestPrice"];
        //        [dic setObject:dic[@"price"] forKey:@"lowestPrice"];
        //        [dic setObject:[NSString stringWithFormat:@"%ld",[socketModel.timestamp longLongValue] / 1000] forKey:@"timestamp"];
        //        [dic setObject:lastModel.close forKey:@"openingPrice"];
        //
        //        [self.originalDataSource insertObject:dic atIndex:0];
        //        [self setData:self.originalDataSource klineType:self.kLine_MainView.klineType timeFormatter:self.formatterString];
        
        socketModel.open = lastModel.close;
        socketModel.close = socketModel.price;
        socketModel.high = socketModel.price;
        socketModel.low = socketModel.price;
        socketModel.timestamp = [NSString stringWithFormat:@"%ld",[socketModel.timestamp longLongValue] / 1000];
...
-(void)longPressEvent:(UILongPressGestureRecognizer *)longPressGesture
{
    ...
    CGPoint point = [longPressGesture locationInView:self.scrollView];
...
#pragma mark - k线图绘制回调 LXY_KLine_VolumeView_Porotcol
