...
-(UILabel *)buyTitleLabel
{
    if (nil == _buyTitleLabel) {
        _buyTitleLabel = [WLTools allocLabel:@"买盘" font:systemFont(ScaleW(10)) textColor:[UIColor lxy_volumeColor] frame:CGRectMake(self.width / 2 - ScaleW(25), kTop_gap, ScaleW(25), ScaleW(10)) textAlignment:NSTextAlignmentCenter];
    }
...
-(UIView *)buyColorView
{
    if (nil == _buyColorView) {
        ...
        _buyColorView.backgroundColor = [UIColor lxy_fallColor];
...
-(UIView *)sellColorView
{
    if (nil == _sellColorView) {
        ...
        _sellColorView.backgroundColor = [UIColor lxy_riseColor];
...
-(UILabel *)sellTitleLabel
{
    if (nil == _sellTitleLabel) {
        _sellTitleLabel = [WLTools allocLabel:@"卖盘" font:systemFont(ScaleW(10)) textColor:[UIColor lxy_volumeColor] frame:CGRectMake(self.sellColorView.right + ScaleW(8), self.buyTitleLabel.y, ScaleW(25), ScaleW(10)) textAlignment:NSTextAlignmentCenter];
...
{
    self.buyModelArray = [LXY_DeepModel mj_objectArrayWithKeyValuesArray:data[@"asks"]];
    
    self.sellModelArray = [LXY_DeepModel mj_objectArrayWithKeyValuesArray:data[@"bids"]];
...
-(void)drawPrice
{
    ...
    LXY_DeepPosition_Model *centerPositionModel = self.buyPositionArray.lastObject;
    LXY_DeepModel *centerModel = self.buyModelArray.lastObject;
    NSString *centerPriceString = [NSString stringWithFormat:@"%.1f",centerModel.price.doubleValue];
    CGFloat centerWidth = [WLTools getWidthWithText:centerPriceString font:self.longPressVolumeLabel.font] + ScaleW(5);
    ...
    [centerPriceString drawInRect:CGRectMake(centerPositionModel.centerX - centerWidth / 2, self.height - kBottom_gap + (kBottom_gap - 10) / 2, centerWidth,10) withAttributes:attribute];
