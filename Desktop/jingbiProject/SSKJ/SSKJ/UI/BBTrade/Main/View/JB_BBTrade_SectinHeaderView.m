...
-(UIView *)vLineView
{
    if (nil == _vLineView) {
        ...
        _vLineView.backgroundColor = kTextLightWhiteColor;
    }
    return _vLineView;
}
...
-(UILabel *)entrustLabel
{
    if (nil == _entrustLabel) {
        _entrustLabel = [WLTools allocLabel:SSKJLocalized(@"当前委托", nil) font:systemFont(15) textColor:kTextLightWhiteColor frame:CGRectMake(self.vLineView.right + ScaleW(10), 0, ScaleW(100), self.backView.height) textAlignment:NSTextAlignmentLeft];
    }
...
-(UIButton *)allOrderButton
{
    if (nil == _allOrderButton) {
        ...
        NSString *title = SSKJLocalized(@"全部订单", nil);
