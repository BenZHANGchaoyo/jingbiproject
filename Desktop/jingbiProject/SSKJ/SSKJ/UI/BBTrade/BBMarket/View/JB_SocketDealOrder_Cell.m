...
-(void)addLabels
{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        UILabel *label = [WLTools allocLabel:title font:systemFont(ScaleW(13)) textColor:kMainWihteColor frame:CGRectMake(ScaleW(15) + i * width, 0, width, self.height) textAlignment:NSTextAlignmentLeft];
...
-(void)setCellWithModel:(JB_BBTrade_SocketDealOrder_Model *)model
{
    ...
    if ([model.dc isEqualToString:@"buy"]) {
        self.typeLabel.text = SSKJLocalized(@"买入", nil);
        self.typeLabel.textColor = GREEN_HEX_COLOR;
    }else{
        self.typeLabel.text = SSKJLocalized(@"卖出", nil);
    ...
    self.amountLabel.text = [WLTools noroundingStringWith:model.amount.doubleValue afterPointNumber:4];
}
