...
{
    if (nil == _mainLabel) {
        _mainLabel = [WLTools allocLabel:SSKJLocalized(@"主图", nil) font:systemFont(ScaleW(13)) textColor:kTextLightBlueColor frame:CGRectMake(ScaleW(0), ScaleW(18), ScaleW(53), ScaleW(13)) textAlignment:NSTextAlignmentCenter];
...
-(UIView *)mainLineView
{
    if (nil == _mainLineView) {
        ...
        _mainLineView.backgroundColor = kTextGrayBlueColor;
    }
...
-(void)addMainAccessory
{
    ...
    for (int i = 0; i < self.mainAccessoryArray.count; i++) {
        ...
        [btn setTitleColor:kTextLightBlueColor forState:UIControlStateNormal];
...
{
    if (nil == _subLabel) {
        _subLabel = [WLTools allocLabel:SSKJLocalized(@"副图", nil) font:systemFont(ScaleW(13)) textColor:kTextLightBlueColor frame:CGRectMake(ScaleW(0), ScaleW(18), ScaleW(53), ScaleW(13)) textAlignment:NSTextAlignmentCenter];
...
-(UIView *)subLineView
{
    if (nil == _subLineView) {
        ...
        _subLineView.backgroundColor = kTextGrayBlueColor;
    }
    return _subLineView;
}
