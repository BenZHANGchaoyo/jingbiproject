...
//        NSInteger status = [ManagerSocket sharedManager].model.auth_status.integerValue;
        NSInteger status = 1;
        if (status == 1) {
            statusString = SSKJLocalized(@"未认证", nil);
        }else if(status == 2){
...
//    NSInteger status = [ManagerSocket sharedManager].model.status.integerValue;
    ...
    if (status == 1) {
        ...
        [MBProgressHUD showError:SSKJLocalized(@"一级认证审核中，请耐心等待", nil)];
        return;
    }else if (status == 3){
...
//    if ([ManagerSocket sharedManager].model.status.integerValue != 3) {
////        [self judgeFristCertificate];
//        [MBProgressHUD showError:ZBLocalized(@"请先进行一级认证", nil)];
//        return;
//    }
