...
-(UILabel *)nameLabel
{
    if (!_nameLabel) {
        ...
        [self label:_nameLabel font:14 textColor:kMainTextColor text:@"姓名"];
...
-(UITextField *)inputTextView{
    if (!_inputTextView) {
        ...
        [self textField:_inputTextView textFont:14 placeHolderFont:14 text:@"" placeText:@"" textColor:kMainTextColor placeHolderTextColor:UIColorFromRGB(0x5b5e95)];
...
{
    if (!_upImgView) {
        _upImgView = [[UIView alloc]initWithFrame:CGRectMake(16,_nameLabel.bottom + 12, Screen_Width - 50 - 32, 80.f)];
        [_upImgView setCornerRadius:5.f];
        [_upImgView setBorderWithWidth:1.f andColor:UIColorFromRGB(0x5b5e95)];
        _upImgView.backgroundColor = [UIColor clearColor];
        ...
        self.selectImg = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"sctp"]];
        self.selectImg.centerX = _upImgView.width/2;
        self.selectImg.y = 10;
        [_upImgView addSubview:_selectImg];
        _messageLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, _selectImg.bottom +10, _upImgView.width, 12)];
        [self label:_messageLabel font:12 textColor:UIColorFromRGB(0x5b5e95) text:@"请"];
...
-(void)setValueWith:(NSDictionary *)dic type:(NSString *)type
{
    ...
    if ([string isEqualToString:SSKJLocalized(@"姓名", nil)]||[string isEqualToString:SSKJLocalized(@"二维码", nil)]) {
        ...
        {
//            _inputTextView.text =  [ManagerSocket sharedManager].model.realname;
            _inputTextView.backgroundColor = UIColorFromRGB(0x5b5e95);
        }
