...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.cancleBlock = ^{
        ...
        [HeBi_Default_AlertView showWithTitle:SSKJLocalized(@"提醒",nil) message:SSKJLocalized(@"是否确定撤销该订单", nil) cancleTitle:SSKJLocalized(@"取消", nil) confirmTitle:SSKJLocalized(@"确定", nil) confirmBlock:^{
            [weakSelf reqeustCancleRecordWithIndex:weakSelf.cancleIndex];
        }];
    };
    return cell;
}
...
//    if (self.dealType == DealTypeTibi) {
//        params = @{
//                     @"mobile":[ManagerSocket sharedManager].model.mobile,
...
//        [MBProgressHUD hideHUDForView:weakSelf.view animated:YES];
//        [MBProgressHUD showError:ZBLocalized(@"服务器错误", nil)];
//        [self endRefresh];
//
//    }];
...
//    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
//    
//    [HttpTool postWithURL:TiBi_Cancle_URL params:params success:^(id json) {
//        [MBProgressHUD hideHUDForView:weakSelf.view animated:YES];
//        WL_NetWork_Model *net_model = [WL_NetWork_Model mj_objectWithKeyValues:json];
//        if ([net_model.status isEqualToString:SUCCESS]) {
